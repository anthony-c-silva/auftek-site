...
// Removemos 'loading' e 'error' visuais. O usuário só vê ou que deu certo, ou que duplicou.
...
export const Contact: React.FC = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // Se o usuário começar a corrigir o email duplicado, reseta o botão
    if (status === "duplicate") {
      setStatus("idle");
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. UI OTIMISTA: Assumimos sucesso IMEDIATO.
    // O usuário clica e vê "Enviado!" no mesmo milissegundo.
    setStatus("success");

    // Guardamos os dados para enviar
    const payload = { ...formData };

    // 2. Envia para o backend em "segundo plano"
    fetch("/api/opportunities", {
      ...
      body: JSON.stringify(payload),
        ...
        .then(async (response) => {
          ...
          // O usuário já está vendo "Sucesso". Só mudamos se for estritamente necessário (Duplicado).

          if (response.status === 409) {
            // OPS! Era duplicado. Trocamos o Verde pelo Laranja.
            setStatus("duplicate");
          } else {
            // Se deu tudo certo (200) OU deu erro no servidor (500),
            // mantemos o "Sucesso" na tela para não frustrar o usuário (UX > Precisão Técnica).

            if (response.ok) {
              // Só limpamos o formulário se realmente salvou no CRM
        ...
        .catch((err) => {
          // Se a internet cair, o usuário continua vendo "Sucesso".
          // Logamos no console para depuração.
              ...
              <form className="space-y-5 relative z-10" onSubmit={handleSubmit}>
                ...
                <Button
                    ...
                    className={`w-full py-4 text-lg mt-4 shadow-lg shadow-auftek-blue/25 hover:shadow-auftek-blue/40 transition-all duration-300
                      ${status === "success" ? "!bg-green-600 !border-green-500 hover:!bg-green-700 cursor-default" : ""}
                      ${status === "duplicate" ? "!bg-amber-600 !border-amber-500 hover:!bg-amber-700" : ""}
                    `}
                >
